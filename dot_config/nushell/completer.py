...
async def main():
    ...
    parser.add_argument(
        "--dunst", action="store_true", help="Use dunstify to show notifications"
    )
    # parser.add_argument(
    #     "--max-files",
    #     "-f",
    ...
    parser.add_argument(
        ...
        type=str,
        default="nushell",
        help="Shell name. Only given to GPT",
    )
    ...
    parser.add_argument(
        ...
        help="GPT model to use, gpt-4 or got-3.5-turbo",
    )
    ...
    if term_content:
        ...
        logging.info(
            f"cleaned_term_content from {len(term_content)} -> {cleaned_term_content}"
        )
        # Log token count as well
        logging.info(f"term_content token count: {count_text_tokens(term_content)}")
        logging.info(
            f"cleaned_term_content token count: {count_text_tokens(cleaned_term_content)}"
        )
        ...
        if args.dunst:
            subprocess.run(
                [
                    ...
                    "-r",
                    str(dunst_id),
                    f"Too many tokens, {tokens} > {MAX_TOKENS}",
                    f"{cwd}\n\n{cmdline}",
                ]
            )
        return
